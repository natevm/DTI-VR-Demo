...
namespace Components::Materials::SurfaceMaterials {
	class TexCoord: public Material {
	public:
		...
		void render(int renderpass, GLuint cameraUBO, std::shared_ptr<Components::Meshes::Mesh> mesh) {
			...
			if (!initialized) {
				std::cout << "TexCoord: material not initialized, not rendering!" << std::endl;
				return;
			}

			if (!mesh) {
				std::cout << "TexCoord: mesh is empty, not rendering!" << std::endl;
				return;
			}
	...
	private:
		...
		static std::vector<TexCoord> TexCoords;
		...
		static void setupGraphicsPipeline() {
			...
			auto vertShaderCode = readFile(ResourcePath "Materials/SurfaceMaterials/TexCoord/shader.vert");
			auto fragShaderCode = readFile(ResourcePath "Materials/SurfaceMaterials/TexCoord/shader.frag");
